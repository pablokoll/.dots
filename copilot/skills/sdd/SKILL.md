...
## How to dispatch
Custom agents live in `.github/agents/`. Dispatch a subagent by spawning a fresh non-interactive session with the full prompt:

```bash
copilot -p "<full prompt>" --agent implementer --allow-all-tools -s
```

Paste the complete task text into the prompt — the subagent must never read `Tasks.md` itself.
